  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div><div className="page-title" style={{display:'flex',alignItems:'center',gap:'0.5rem'}}><Newspaper /> {t("news_title")}</div><div className="page-sub">{t("news_sub")}</div></div>
      ...
      <div className="card">
        ...
        : news.length === 0 ? (
          <div className="empty-state"><div className="empty-icon"><Newspaper size={48} /></div><p><Link to="/admin/news/new" style={{color:"var(--red)"}}>{t("new_article")}</Link></p></div>
        ) : (
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {news.map((n) => (
                  <tr key={n.id}>
                    <td>{n.image_url ? <img src={n.image_url} alt="" className="img-preview" /> : <div style={{width:60,height:45,background:"var(--bg)",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}><Newspaper /></div>}</td>
                    ...
                    <td>
                      <div style={{display:"flex",gap:8}}>
                        <Link to={`/admin/news/${n.id}`} className="btn btn-secondary btn-sm"><Edit2 size={16} /></Link>
                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(n.id)}><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
